...
export default function Navbar() {
  ...
  const linkColor = scrolled ? "text-gray-700 hover:text-blue-600" : "text-white/90 hover:text-white"
  ...
  return (
    <nav
      ...
      style={scrolled ? { borderBottom: "1px solid #e8eeff" } : {}}
    >
              ...
              <div className="flex items-center gap-3">
                {isAdmin && (
                  <button onClick={() => navigate("/admin")}
                    className={`flex items-center gap-1 text-sm font-semibold transition-colors ${scrolled ? "text-purple-600 hover:text-purple-800" : "text-purple-300 hover:text-white"}`}>
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-3 shadow-lg">
          {navLinks.map((l) => (
            <a key={l.label} href={l.href} onClick={() => setOpen(false)}
              className="block text-gray-700 font-medium py-1 hover:text-blue-600 transition-colors">
              {l.label}
            </a>
          ))}
