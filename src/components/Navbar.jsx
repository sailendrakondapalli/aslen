        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              <span className="font-black text-gray-900 text-lg tracking-tight">ASLEN</span>
              <span className="hidden sm:inline text-gray-400 text-xs ml-1">TECH SOLUTIONS</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <div className="flex items-center gap-3">
                {isAdmin && (
                  <button onClick={() => navigate('/admin')} className="flex items-center gap-1 text-sm text-purple-600 hover:text-purple-800 font-semibold transition-colors">
                    <ShieldCheck size={15} /> Admin
                  </button>
                )}
                <button onClick={() => navigate('/dashboard')} className="flex items-center gap-1 text-sm text-gray-700 hover:text-blue-600 transition-colors">
                  <LayoutDashboard size={15} /> Dashboard
                </button>
                <img
                  src={user.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.user_metadata?.full_name || 'U')}&background=3b82f6&color=fff`}
                  alt="avatar"
                  className="w-8 h-8 rounded-full object-cover border-2 border-blue-200 cursor-pointer"
          ...
          <div className="pt-2 border-t border-gray-100">
            {user ? (
              <div className="space-y-2">
                {isAdmin && (
                  <button onClick={() => { navigate('/admin'); setOpen(false) }} className="flex items-center gap-2 text-purple-600 font-semibold w-full">
                    <ShieldCheck size={16} /> Admin Panel
                  </button>
                )}
                <button onClick={() => { navigate('/dashboard'); setOpen(false) }} className="flex items-center gap-2 text-gray-700 font-medium w-full">
                  <LayoutDashboard size={16} /> Dashboard
                </button>
                <button onClick={handleSignOut} className="flex items-center gap-2 text-red-500 font-medium w-full">
